<?php
/**
 * Media & document library with category chips and in-page search.
 *
 * @package DTH
 */
...
$dth_terms = get_terms( array( 'taxonomy' => 'dth_media_cat', 'hide_empty' => true ) );
...
<section class="block" style="padding-top:clamp(20px,2.5vw,30px)">
  <div class="wrap">
	...
	<?php if ( $dth_terms ) : ?>
    <div class="tabbar" id="catBar" aria-label="<?php esc_attr_e( 'หมวดหมู่สื่อ', 'dth' ); ?>" style="margin-bottom:clamp(18px,2.5vw,26px)">
    ...
    <div class="hub-toolbar">
      <div class="hub-search">
        <input type="text" id="hubSearch" placeholder="<?php esc_attr_e( 'ใส่คำที่ต้องการค้นหา…', 'dth' ); ?>" aria-label="<?php esc_attr_e( 'ค้นหาสื่อและเอกสาร', 'dth' ); ?>">
    ...
    <div class="pager">
      <div class="total"><?php esc_html_e( 'จำนวนทั้งหมด', 'dth' ); ?> <b id="hubCount"><?php echo esc_html( $GLOBALS['wp_query']->post_count ); ?></b> <?php esc_html_e( 'รายการ', 'dth' ); ?></div>
		...
		<?php dth_pagination(); ?>
	<?php else : ?>
    <div class="card dth-empty"><p><?php esc_html_e( 'ยังไม่มีสื่อหรือเอกสารในขณะนี้', 'dth' ); ?></p></div>
	<?php endif; ?>
  </div>
</section>
</main>
