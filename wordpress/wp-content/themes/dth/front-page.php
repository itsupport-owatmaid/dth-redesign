...
<script>
  ...
  // ---- Hero slider (ทำงานเฉพาะเมื่อมี markup สไลด์อยู่ในเนื้อหาหน้าแรก) ----
  (()=>{
    const slides=[...document.querySelectorAll('.slide')],dots=document.getElementById('dots');
    if(!slides.length)return;
    let i=0,timer;
    slides.forEach((s,idx)=>{if(!dots)return;const d=document.createElement('button');d.setAttribute('role','tab');d.setAttribute('aria-label','สไลด์ '+(idx+1));if(idx===0)d.classList.add('on');d.addEventListener('click',()=>go(idx));dots.appendChild(d);});
    function go(n){slides[i].classList.remove('on');if(dots&&dots.children[i])dots.children[i].classList.remove('on');i=(n+slides.length)%slides.length;slides[i].classList.add('on');if(dots&&dots.children[i])dots.children[i].classList.add('on');}
    const reduce=matchMedia('(prefers-reduced-motion:reduce)').matches;
    function start(){if(!reduce&&slides.length>1)timer=setInterval(()=>go(i+1),5500);}
    function stop(){clearInterval(timer);}
    const next=document.getElementById('next'),prev=document.getElementById('prev');
    if(next)next.addEventListener('click',()=>{go(i+1);stop();start();});
    if(prev)prev.addEventListener('click',()=>{go(i-1);stop();start();});
